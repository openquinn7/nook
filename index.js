...
const { GachaSystem, GACHA_TYPES, GACHA_PITY_SYSTEM } = require('./src/gacha');
const fs = require('fs');
...
class NookProtocol {
  constructor(options = {}) {
    ...
    this.gachaSystem = new GachaSystem(this.cosmeticSystem);
   ...
   * Roll gacha
   */
  rollGacha(type = 'SINGLE', currency = 'sparks') {
    const balance = currency === 'sparks' ? this.sparkEngine.getBalance() : 0;
    const result = this.gachaSystem.roll(type, currency, balance);

    // Add to owned cosmetics
    for (const item of result.results) {
      if (item.isNew) {
        const category = this.cosmeticSystem.getCategoryForCosmetic(item.cosmetic.id);
        if (category) {
          this.cosmeticSystem.owned[category].push(item.cosmetic);
        }
      }
    }
